...
func TestOperations_ReadFiles(t *testing.T) {
	...
	assert.Equal(t, "content-aaa", buffers[0].String())
	assert.Equal(t, "content-ccc", buffers[1].String())
}
...
func TestOperations_ReadFileNames(t *testing.T) {
	...
	assert.Equal(t, filepath.Join(expectedDir, "aaa.keep"), names[0])
	assert.Equal(t, filepath.Join(expectedDir, "ccc.keep"), names[1])
}
