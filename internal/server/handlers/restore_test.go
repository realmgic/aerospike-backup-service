...
func TestService_RestoreFullHandler(t *testing.T) {
	...
	mockManager.EXPECT().Restore(gomock.Any(), gomock.Any()).Return(model.RestoreJobID(1), nil)
	...
	req := httptest.NewRequestWithContext(t.Context(), http.MethodPost, "/v1/restore/full", strings.NewReader(body))
	w := httptest.NewRecorder()

	svc.RestoreFullHandler(w, req)
...
func TestService_RestoreIncrementalHandler(t *testing.T) {
	...
	mockManager := service.NewMockRestoreManager(ctrl)
	mockManager.EXPECT().Restore(gomock.Any(), gomock.Any()).Return(model.RestoreJobID(2), nil)
	...
	req := httptest.NewRequestWithContext(
		t.Context(), http.MethodPost, "/v1/restore/incremental", strings.NewReader(body),
	)
	w := httptest.NewRecorder()

	svc.RestoreIncrementalHandler(w, req)
...
func TestService_RestoreByTimeHandler(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			requestBody:    marshalToString(dto.RestoreTimestampRequest{Routine: "daily", Policy: &dto.TimestampRestorePolicy{}}),
			setupMock:      func(*service.MockRestoreManager) {},
			expectedStatus: http.StatusBadRequest,
		},
		{
